...
async function main() {
  ...
  // --- NUEVO: Pedir y reemplazar los ARN en el workflow de GitHub Actions ---
  ...
  if (fs.existsSync(deployWorkflowTemplatePath)) {
    // Copia el archivo template a deploy.yml
    fs.copyFileSync(deployWorkflowTemplatePath, deployWorkflowPath);
    const devArn = await askQuestion(`${colors.green}Enter the DEV_ROLE_ARN (AWS ARN for development): ${colors.reset}`);
    const prodArn = await askQuestion(`${colors.green}Enter the PROD_ROLE_ARN (AWS ARN for production): ${colors.reset}`);
    let workflowContent = fs.readFileSync(deployWorkflowPath, 'utf8');
    workflowContent = workflowContent.replace(/DEV_ROLE_ARN: .*/,      `DEV_ROLE_ARN: ${devArn.trim()}`);
    workflowContent = workflowContent.replace(/PROD_ROLE_ARN: .*/,      `PROD_ROLE_ARN: ${prodArn.trim()}`);
    fs.writeFileSync(deployWorkflowPath, workflowContent, 'utf8');
    console.log(`${colors.green}GitHub Actions workflow has been created and updated with provided ARNs!${colors.reset}`);
  } else {
